...
public class AccountDaoImpl implements AccountDao {
    ...
    public void updateAccount(Account account) throws SQLException {
        ...
        qr.update(ManagerTheadLocal.getConnection(),"update test set money=? where name=?",account.getMoney(),account.getName());
    }

    @Override
    public Account findAccountByname(String name) throws SQLException {
        QueryRunner qr = new QueryRunner();
        return qr.query(ManagerTheadLocal.getConnection(),"select * from test where name=?",new BeanHandler<Account>(Account.class),name);
